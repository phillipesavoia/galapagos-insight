...
import { BulkPasteNav } from "./BulkPasteNav";
import { FetchMarketPrices } from "./FetchMarketPrices";
...
export function NavsTab() {
  ...
  const fetchNavs = async () => {
    ...
    setNavData(dates.map((date) => ({ date, ...dateMap[date] })));

    const chartDates = Object.keys(dateMap).sort().slice(-60);
    setChartData(chartDates.map((date) => ({ date: date.slice(5), ...dateMap[date] })));
    setLoading(false);
  };
  ...
  const processFile = async (file: File) => {
    ...
    Papa.parse(file, {
          ...
          for (const row of results.data as Record<string, string>[]) {
            ...
            for (const portfolio of portfolios) {
              ...
              if (val !== undefined && val !== "") {
                ...
                if (!isNaN(nav)) rows.push({ date: dateStr, portfolio_name: portfolio, nav });
              }
            }
          }
          if (rows.length === 0) { toast.error("Nenhum dado válido encontrado no CSV"); setUploading(false); return; }
          const batchSize = 500;
          for (let i = 0; i < rows.length; i += batchSize) {
            const batch = rows.slice(i, i + batchSize);
            const { error } = await supabase.from("daily_navs").upsert(
              batch.map((r) => ({ date: r.date, portfolio_name: r.portfolio_name, nav: r.nav })),
      ...
      error: (err) => { console.error("Papa parse error:", err); toast.error("Erro ao ler arquivo CSV"); setUploading(false); },
    });
  };
  ...
  return (
    <div className="space-y-8">
      {/* Bulk Paste Area */}
      <BulkPasteNav onSaved={fetchNavs} />
      ...
      <div
        ...
        className={`relative border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all duration-200 ${
          dragActive ? "border-primary bg-primary/5" : "border-border hover:border-primary/40 hover:bg-card/50"
        }`}
      >
        ...
        <div className="flex flex-col items-center gap-2">
          <div className="h-10 w-10 rounded-xl bg-primary/10 flex items-center justify-center">
            {uploading ? <Loader2 className="h-5 w-5 text-primary animate-spin" /> :
             uploadResult?.success ? <CheckCircle2 className="h-5 w-5 text-primary" /> :
             uploadResult && !uploadResult.success ? <AlertCircle className="h-5 w-5 text-destructive" /> :
             <Upload className="h-5 w-5 text-primary" strokeWidth={1.5} />}
          </div>
          <p className="text-xs font-medium text-foreground">
            {uploading ? "Processando..." : uploadResult?.success ? `${uploadResult.count} registros importados` : "Ou arraste um arquivo CSV aqui"}
          </p>
          <p className="text-xs text-muted-foreground">CSV · Date | Liquidity | Bonds | Conservative | Income | Balanced | Growth</p>
        </div>
      </div>
        ...
        <>
              ...
              <div className="h-56">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData}>
                    ...
                    <Tooltip contentStyle={{ background: "hsl(240,15%,7%)", border: "1px solid hsl(240,10%,16%)", borderRadius: "8px", fontSize: "12px" }} />
                    {portfolios.map((p, i) => <Line key={p} type="monotone" dataKey={p} stroke={lineColors[i]} strokeWidth={1.5} dot={false} />)}
                  </LineChart>
                </ResponsiveContainer>
              </div>
          ...
          {navData.length > 0 ? (
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-secondary/50 hover:bg-secondary/50">
                    ...
                    {portfolios.map((p) => <TableHead key={p} className="text-xs font-semibold text-muted-foreground uppercase tracking-wider text-right">{p}</TableHead>)}
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {navData.map((row) => (
            ...
            <div className="text-center py-12 text-muted-foreground text-sm">
              Nenhum dado de NAV encontrado. Use o Bulk Paste ou faça upload de um CSV.
            </div>
          )}
        </>
      )}
