  ...
  return (
    <Layout>
      <div className="flex h-screen bg-background">
        {/* History Sidebar */}
        {showHistory && (
          <div className="w-64 border-r border-border bg-ares-bg-surface flex flex-col shrink-0">
            <div className="px-4 pt-5 pb-2">
              <button
                onClick={handleNewChat}
                className="flex items-center gap-3 text-sm text-muted-foreground hover:text-foreground hover:bg-secondary rounded-full px-4 py-2.5 transition-colors w-full"
              >
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin px-2 pb-2 space-y-0.5">
                ...
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-full text-[13px] transition-colors truncate ${
                    s.session_id === sessionId
                      ? "bg-ares-green/10 text-ares-green font-medium"
              ...
              {sessions.length === 0 && (
                <p className="text-xs text-muted-foreground px-3 py-4 text-center">Nenhuma conversa anterior.</p>
              )}
            </div>
          </div>
        )}
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {/* Fixed header */}
          <div className="h-10 border-b border-border bg-ares-bg-surface flex items-center justify-between px-4 shrink-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-ares-orange font-medium tracking-widest uppercase">Offshore</span>
            </div>
            <div className="flex items-center gap-2">
              {activePortfolio && (
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-ares-green/10 text-ares-green border border-ares-green/20 animate-fade-in">
                  📍 Foco: {activePortfolio}
                  <button
                    onClick={() => setActivePortfolio(null)}
                    className="ml-0.5 text-ares-green/60 hover:text-ares-green"
                  >
                    <X className="h-3 w-3" />
                  </button>
                </span>
              ...
              {activeTicker && !activePortfolio && (
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-ares-orange/10 text-ares-orange border border-ares-orange/20 animate-fade-in">
                  📍 Ativo em Análise: {activeTicker}
                  <button
                    onClick={() => setActiveTicker(null)}
                    className="ml-0.5 text-ares-orange/60 hover:text-ares-orange"
                  >
                    <X className="h-3 w-3" />
                  </button>
                </span>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {randomSuggestions.map((s) => (
                    <button
                      ...
                      className="text-left p-4 rounded-xl border border-border bg-card text-sm text-muted-foreground hover:border-ares-green/30 hover:text-foreground transition-colors"
                    >
                      {s}
                    </button>
                  ))}
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin px-6 py-6 space-y-6 bg-background" ref={messagesEndRef}>
                ...
                <div key={msg.id} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                  <div
                    className={`max-w-4xl w-full text-[13px] leading-[1.7] ${
                      msg.role === "user"
                        ? "bg-secondary text-foreground rounded-2xl px-4 py-3"
                        : "text-foreground"
                    }`}
                  >
                    {msg.role === "assistant" ? (
                      <>
                        {msg.content && (() => {
                          ...
                          return (
                            <>
                              <div className="prose prose-sm prose-invert max-w-none text-foreground/90 [&_p]:my-2 [&_ul]:my-2 [&_ol]:my-2 [&_li]:my-1 [&_strong]:text-foreground [&_strong]:font-semibold [&_h1]:text-[15px] [&_h2]:text-[14px] [&_h3]:text-[13px] [&_h1]:font-bold [&_h2]:font-bold [&_h3]:font-semibold [&_h1]:mt-4 [&_h2]:mt-4 [&_h3]:mt-3 [&_h1]:mb-2 [&_h2]:mb-2 [&_h3]:mb-1 [&_ul]:pl-5 [&_ol]:pl-5 [&_li]:text-foreground/80 [&_hr]:my-3 [&_hr]:border-border">
                                ...
                                <div className="mt-3 flex flex-col gap-1.5">
                                  {followUps.map((q, i) => (
                                    <button
                                      ...
                                      disabled={isLoading}
                                      className="text-left px-3 py-1.5 rounded-lg text-xs text-ares-green bg-ares-green/5 border border-ares-green/20 hover:bg-ares-green/10 hover:border-ares-green/30 transition-colors disabled:opacity-50 leading-snug"
                                    >
                                      {q}
                                    </button>
                                  ))}
                        ...
                        {msg.toolPending && (
                          <div className="mt-3 flex items-center gap-2 text-xs text-ares-orange bg-ares-orange/5 border border-ares-orange/20 rounded-lg px-3 py-2 animate-pulse">
                    ...
                    {msg.role === "assistant" && (msg.content || (msg.toolCalls && msg.toolCalls.length > 0)) && (
                      <div className="flex items-center gap-2 mt-3 pt-2 border-t border-border/50">
                        ...
                        <div className="flex items-center gap-0.5">
                          <button onClick={() => {}} className="p-1 rounded-md text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors" title="Útil">
                            <ThumbsUp className="h-3 w-3" strokeWidth={1.5} />
                          </button>
                          <button onClick={() => {}} className="p-1 rounded-md text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors" title="Não útil">
                          ...
                          <button
                            ...
                            className="p-1 rounded-md text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors" title="Regenerar"
                          >
                            <RefreshCw className="h-3 w-3" strokeWidth={1.5} />
                          </button>
                          <button onClick={() => navigator.clipboard.writeText(msg.content)} className="p-1 rounded-md text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors" title="Copiar">
                        ...
                        {msg.sources.map((src, i) => (
                          src.file_url ? (
                            <a key={i} href={src.file_url} target="_blank" rel="noopener noreferrer"
                              className="inline-flex items-center px-2.5 py-1 rounded-md bg-ares-orange/5 text-xs text-ares-orange border border-ares-orange/20 hover:bg-ares-orange/10 hover:border-ares-orange/30 transition-colors cursor-pointer">
                              {src.name} · {src.period}
                            </a>
                          ) : (
                            <span key={i} className="inline-flex items-center px-2.5 py-1 rounded-md bg-secondary text-xs text-muted-foreground border border-border">
                              {src.name} · {src.period}
                            </span>
                          )
                        ))}
              ...
              {isLoading && (
                <div className="flex justify-start animate-fade-in">
                  <div className="bg-secondary border border-border rounded-2xl px-4 py-3 flex items-center gap-1.5">
                    <span className="h-2 w-2 rounded-full bg-ares-green animate-[pulse_1s_ease-in-out_infinite]" />
                    <span className="h-2 w-2 rounded-full bg-ares-green animate-[pulse_1s_ease-in-out_0.2s_infinite]" />
                    <span className="h-2 w-2 rounded-full bg-ares-green animate-[pulse_1s_ease-in-out_0.4s_infinite]" />
                  </div>
                </div>
              )}
            </div>
          ...
          {/* Input */}
          <div className="border-t border-border p-4 bg-ares-bg-surface">
            <div className="flex items-end gap-2">
              <button
                onClick={() => setShowHistory(!showHistory)}
                className={`h-11 w-11 rounded-xl flex items-center justify-center transition-colors shrink-0 ${showHistory ? 'text-ares-green bg-ares-green/10 border border-ares-green/20' : 'text-muted-foreground bg-secondary border border-border hover:text-foreground hover:bg-secondary/80'}`}
              ...
              <textarea
                ...
                rows={1}
                className="flex-1 resize-none bg-secondary border border-border rounded-xl px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ares-green/50"
              />
              <button
                onClick={() => handleSend()}
                className="h-11 w-11 rounded-xl bg-ares-green text-primary-foreground flex items-center justify-center hover:bg-ares-green/90 transition-colors shrink-0"
              >
